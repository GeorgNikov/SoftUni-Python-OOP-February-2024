...
class Everland:
    ...
    def add_room(self, room: Room):
        ...
        self.total_people += room.members_count
    ...
    def pay(self):
        ...
        for r in self.rooms[:]:
            ...
            if r.budget >= total_cost:
                ...
                self.rooms.remove(r)
                self.total_people -= r.members_count

        return "\n".join(output)

    def status(self):
        output = [f"Total population: {self.total_people}"]
        c_cost = 0
        for r in self.rooms:
            output.append(
            ...
            for n, c in enumerate(r.children, 1):
                ...
                c_cost += c.get_monthly_expense()
            output.append(f"--- Appliances monthly cost: {r.expenses - c_cost:.2f}$")
        return "\n".join(output)